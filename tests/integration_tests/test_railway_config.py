"""Integration tests for django-simple-deploy, targeting Railway."""
...
def test_settings(tmp_project):
    ...
    again.
    """
    hf.check_reference_file(tmp_project, "blog/settings.py", "dsd-railway")
...
def test_pipfile(tmp_project, pkg_manager, tmp_path, dsd_version):
    ...
    if pkg_manager in ("req_txt", "poetry"):
        ...
        context = {"current-version": dsd_version}
        hf.check_reference_file(
            tmp_project, "Pipfile", "dsd-railway", context=context, tmp_path=tmp_path
        )


def test_gitignore(tmp_project):
    """Test that .gitignore has been modified correctly."""
    hf.check_reference_file(tmp_project, ".gitignore", "dsd-railway")


# --- Test Railway-specific files ---

def test_creates_railway_toml(tmp_project, pkg_manager):
...
def test_log_dir(tmp_project):
    ...
    assert "INFO: Configuring project for deployment to Railway..." in log_file_text

    assert "INFO: CLI args:" in log_file_text
    assert (
        "INFO: Deployment target: Railway" in log_file_text
        or "INFO: Deployment target: Railway" in log_file_text
    )
    assert "INFO:   Using plugin: dsd_railway" in log_file_text
    ...
    assert (
        "INFO: --- Your project is now configured for deployment on Railway ---"
        in log_file_text
    )
